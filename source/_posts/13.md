...
fn setup(mut commands: Commands) {
    ...
    println!("\n🎮 Raven Test 启动");
    println!("📂 资源目录: assets/images/");
    println!("========== 快捷键说明 ==========");
    println!("  L  → 开始加载所有资源");
    println!("  M  → 切换内存显示UI");
    println!("  1-4 → 切换背景图片");
    println!("  Q  → Sylvie蓝色-普通");
    println!("  W  → Sylvie蓝色-微笑");
    println!("  E  → Sylvie绿色-普通");
    println!("  R  → Sylvie绿色-咯咯笑");
    println!("  C  → 清除所有显示");
    println!("===============================\n");
}
...
fn load_assets_system(
    ...
    if input.just_pressed(KeyCode::KeyL) && assets.loading_status == LoadingStatus::NotStarted {
        ...
        println!("   ✅ 已请求加载4张蓝色Sylvie");
        
        // 加载Sylvie绿色系列
        println!("👥 加载Sylvie绿色系列...");
        ...
        println!("   ✅ 已请求加载4张绿色Sylvie");
        
        println!("📊 总计: 12张图片已开始异步加载\n");
    }
}
...
fn memory_monitor_system(
    ...
    if current_time - monitor.last_update > 0.5 {
        ...
        if (current_time as i32) % 2 == 0 && current_time - monitor.last_update < 0.6 {
            println!("========== 内存监控报告 ==========");
            println!("⏱️  运行时间: {:.1}秒", current_time);
            println!("🎮 GPU显存使用: {:.1} MB ({} 张图片)", gpu_mb, image_count);
            
            for (i, (w, h, mb)) in image_details.iter().enumerate() {
                println!("   🖼️  图片{}: {}×{} = {:.2}MB", i + 1, w, h, mb);
            }
            ...
            if monitor.memory_history.len() > 1 {
                ...
                let trend = if change > 0.1 { "📈 上升" } 
                           else if change < -0.1 { "📉 下降" } 
                           else { "➡️  稳定" };
                println!("📊 内存变化: {} ({:+.1}MB)", trend, change);
            }
            
            // 内存警告
            if gpu_mb > 100.0 {
                println!("⚠️  GPU显存使用较高: {:.1}MB", gpu_mb);
            }
            if gpu_mb > 200.0 {
                println!("🚨 GPU显存使用过高: {:.1}MB!", gpu_mb);
            }
...
fn check_loading_completion(assets: &mut RavenAssets, images: &Assets<Image>) {
    let total_expected = 12; // 4背景 + 4蓝色 + 4绿色
    ...
    if loaded_count == total_expected {
        if let Some(start_time) = assets.load_start_time {
            ...
            println!("\n🎉 所有资源加载完成！");
            println!("⏱️  加载耗时: {:.2}秒", elapsed.as_secs_f32());
            println!("✅ 现在可以按 1-4 切换背景，Q-R 切换角色\n");
            assets.loading_status = LoadingStatus::Completed;
        }
    } else {
        println!("⏳ 加载进度: {}/{}", loaded_count, total_expected);
    }
}
...
fn input_handler(
    ...
    if input.just_pressed(KeyCode::KeyM) {
        ...
        println!("🖥️  UI显示: {}", if display_state.show_ui { "✅ 开启" } else { "❌ 关闭" });
    }
...
fn ui_system(
    ...
    // 内存显示
    commands.spawn((
        Text::new(format!("💾 显存: {:.1}MB", current_memory)),
        TextFont {
            font_size: 20.0,
            ..default()
        },
    ...
    let status_text = match assets.loading_status {
        LoadingStatus::NotStarted => "状态: ⏳ 未开始 (按L加载)".to_string(),
        LoadingStatus::Loading => "状态: 🔄 正在加载...".to_string(),
        LoadingStatus::Completed => "状态: ✅ 加载完成".to_string(),
        LoadingStatus::Error(ref e) => format!("状态: ❌ 错误: {}", e),
    };
    ...
    let instructions = if assets.loading_status == LoadingStatus::Completed {
        "L:加载 | M:UI | 1-4:背景 | Q/W/E/R:角色 | C:清除"
    } else {
        "L:开始加载资源 | M:切换UI显示"
    };
    ...
    commands.spawn((
        ...
        TextColor(Color::srgb(0.7, 0.7, 0.7)), // 灰色
        Node {
            position_type: PositionType::Absolute,
            bottom: Val::Px(10.0),
            left: Val::Px(10.0),
